...
class RegionModel:
    ...
    def __init__(self, country_str, region_str, subregion_str,
        ...
        # print("projection_create_date", projection_create_date)
        # print("projection_end_date", projection_end_date)

        self.country_holidays = None
        self.N = (self.projection_end_date - self.first_date).days + 1 

        self.vaccine_efficacy = 0.95
    ...
    def init_params(self, params_tups):
        ...
        # incorporate vaccinations 
        self.vaccination_coverage_arr = self.build_vaccination_coverage_arr()
        self.total_vaccination_arr = self.build_cumulative_vaccination_arr()
    ...
    def build_r_0_arr(self):
        ...
        # Calcualte 'reopen_r` and `post_reopen_r` based on conditions. 
        ...
        # determine key indices for transitions. Variables ending with "idx" represent specific day indices in the simulation timeline. 
        # These indices are used to mark important transition points for the reproduction number R0. 
        # reopen_idx: Marks the day when reopening starts. Used to start the transition from the lockdown R0 to the reopning R0 
        # fatigue_idx: Marks the day when lockdown fatigue sets in.
        # lockdown_reopen_midpoint_idx: Midpoint day between the start of lockdown and reopening.
        # post_reopen_midpoint_idx: Midpoint day during the post-reopening period. 
        # post_reopen_idx: Marks the end of the reopening transition.
        # fall_start_idx: Marks the start of the fall season adjustments to R0   
        # vaccination_start_idx: Marks the start of the vaccination 
        ...
        # print(fatigue_idx, lockdown_reopen_midpoint_idx, reopen_idx, post_reopen_midpoint_idx, post_reopen_idx, fall_start_idx, self.N) 
        # 67 74 110 128 146 185 696 
        try:
            self.use_peak2   
        except AttributeError:
            self.use_peak2 = False

        ## Step 2. Transition Sigmoids  
        ...
        # print(self.rate_of_inflection, 0.2, self.REOPEN_INFLECTION)
        # 0.35245146707754205 0.2 0.32847835689143956
        # print(self.INITIAL_R_0, self.LOCKDOWN_R_0, self.LOCKDOWN_FATIGUE, reopen_r, post_reopen_r) 
        # 2.2637755302375644 0.9038440070216232 1.0 1.2681889965435804 1.0073471671122585 

        ## incorporate vaccinations     
        # optimal: (0.15, 1.1, 1.2, 400, 0.1, 1.2, 1.1)
        if not self.wrap_model:
            vaccination_start_idx = (pd.to_datetime("2020-12-14").date() - self.first_date).days - 8
            sig_post_vaccination = get_transition_sigmoid(
                vaccination_start_idx-5, 0.15, 1.1, 1.2)  
            peak_2_idx = 400
            sig_post_peak_2 = get_transition_sigmoid(
                peak_2_idx, 0.1, 1.2, 1.1)  
        else:
            vaccination_start_idx = (pd.to_datetime("2020-12-14").date() - self.first_date).days - 8
            sig_post_vaccination = get_transition_sigmoid(
                vaccination_start_idx-5, self.vaccination_inflation, 
                self.vaccination_high, self.vaccination_low) 
            
            peak_2_idx = self.peak2_idx
            sig_post_peak_2 = get_transition_sigmoid(
                peak_2_idx, self.peak2_inflation, self.peak2_low, self.peak2_high)  
         
     
        dates = utils.date_range(self.first_date, self.projection_end_date)
        assert len(dates) == self.N 

        ## Step 3. Daily R0 calculation 

        # fatigue_idx, lockdown_reopen_midpoint_idx, reopen_idx, post_reopen_midpoint_idx, post_reopen_idx, fall_start_idx, self.N 
        R_0_ARR = [self.INITIAL_R_0]
        for day_idx in range(1, self.N): 
            # Original: Start 
            if day_idx < lockdown_reopen_midpoint_idx: # lockdown_reopen_midpoint_idx = 74
                r_t = sig_lockdown(day_idx)
                if abs(self.LOCKDOWN_FATIGUE - 1) > 1e-9:
                    r_t *= 1 + sig_fatigue(day_idx)
            elif day_idx > post_reopen_midpoint_idx: # post_reopen_midpoint_idx = 146
            ...
            if not self.skip_vaccinations and day_idx > vaccination_start_idx: 
                r_t = sig_post_vaccination(day_idx) 

            if self.use_peak2 and day_idx > peak_2_idx:
                r_t = sig_post_peak_2(day_idx)
            ## attempt 1: End 
        ...
        # print(R_0_ARR[vaccination_start_idx], "check")
        # exit()
    ...
    def build_vaccination_coverage_arr(self): 
        # Load the CSV data into a DataFrame
        # csv_file = '..\\data\\full_vaccinations_data.csv' 
        csv_file = '..\\data\\vaccination_trends_data.csv'
        ...
        vaccination_data = vaccination_data[
            (vaccination_data['Date'] >= self.first_date) & 
            (vaccination_data['Date'] <= self.projection_end_date)
        ]

        # Select 'Administered_Daily' column and convert it to an array
        daily_vaccinations_arr = vaccination_data['Administered_Cumulative'].values  
        full_vaccinations_arr = [0]*self.N
        full_vaccinations_arr[-len(daily_vaccinations_arr):] = daily_vaccinations_arr  
        return np.array(full_vaccinations_arr) 
    
    def build_cumulative_vaccination_arr(self): 
        # Load the CSV data into a DataFrame
        # csv_file = '..\\data\\full_vaccinations_data.csv' 
        csv_file = '..\\data\\vaccination_trends_data.csv'
        ...
        # Select 'Administered_Daily' column and convert it to an array
        daily_vaccinations_arr = vaccination_data['Administered_Daily'].values  
        full_vaccinations_arr = [0]*self.N
        full_vaccinations_arr[-len(daily_vaccinations_arr):] = daily_vaccinations_arr  
        return np.array(full_vaccinations_arr) 
